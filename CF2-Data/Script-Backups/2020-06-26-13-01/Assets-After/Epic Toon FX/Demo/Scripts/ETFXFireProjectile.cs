...
namespace EpicToonFX
{
    public class ETFXFireProjectile : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (ControlFreak2.CF2Input.GetKeyDown(KeyCode.Mouse0)) //On left mouse down-click
            {
                if (!EventSystem.current.IsPointerOverGameObject()) //Checks if the mouse is not over a UI part
                {
                    if (Physics.Raycast(Camera.main.ScreenPointToRay(ControlFreak2.CF2Input.mousePosition), out hit, 100f)) //Finds the point where you click with the mouse
                    {
