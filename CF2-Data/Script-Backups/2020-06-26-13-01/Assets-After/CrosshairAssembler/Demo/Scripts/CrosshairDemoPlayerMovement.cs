...
public class CrosshairDemoPlayerMovement : MonoBehaviour
{
    ...
    void Start()
    {
        ControlFreak2.CFCursor.lockState = CursorLockMode.Locked; // hide cursor
    ...
    {
        // character movement
        float horiz = ControlFreak2.CF2Input.GetAxisRaw("Horizontal");
        float vert = ControlFreak2.CF2Input.GetAxisRaw("Vertical");
        ...
        // character jump
        if (ControlFreak2.CF2Input.GetKeyDown(KeyCode.Space) && playerController.isGrounded)
        {
            jumpingPower = 15;
        }
        else
        ...
        // character rotation
        float mouseX = ControlFreak2.CF2Input.GetAxis("Mouse X") * Sensitivity;
        float mouseY = ControlFreak2.CF2Input.GetAxis("Mouse Y") * Sensitivity;
